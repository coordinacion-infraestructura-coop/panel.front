...
import type {
  ...
  EstadoCCCreate, EstadoCCUpdate, EstadoHistorialCC, GeoLocalidad, PedidoCC,
...
function EditModal({
  municipio, estados, onSave, onClose, isSaving,
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" role="dialog" aria-modal="true" aria-labelledby={`${uid}-t`}>
      <div className="bg-white rounded-lg w-[560px] max-w-[97vw] max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="text-white px-4 py-3 flex items-center gap-3 rounded-t-lg sticky top-0 z-10" style={{ background: '#172c3f' }}>
          <h3 id={`${uid}-t`} className="flex-1 font-semibold text-sm">Agregar municipio — Cordón Cuneta</h3>
          <button onClick={onClose} className="text-sky-300 hover:text-white text-xl leading-none" aria-label="Cerrar">✕</button>
        </div>
        <div className="p-5 space-y-4">
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label htmlFor={`${uid}-mun`} className="block text-xs font-bold text-gray-500 uppercase mb-1">Municipio *</label>
              <input id={`${uid}-mun`} className={inp} value={form.municipio} onChange={(e) => setForm((p) => ({ ...p, municipio: e.target.value }))} />
            </div>
            <div>
        ...
        <div className="px-5 py-3 border-t border-slate-100 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-1.5 rounded text-sm border border-slate-200 text-gray-600 hover:bg-slate-50 transition-colors">Cancelar</button>
          <button onClick={() => createMut.mutate(form)} disabled={createMut.isPending || !form.municipio.trim()} className="px-5 py-1.5 rounded text-sm text-white disabled:opacity-50 transition-colors hover:opacity-90" style={{ background: '#172c3f' }}>
            {createMut.isPending ? 'Guardando...' : 'Agregar municipio'}
          </button>
        </div>
      </div>
    </div>
          ...
          <table className="w-full border-collapse" style={{ fontSize: '12px' }}>
            <thead>
              <tr style={{ background: '#172c3f', color: '#fff' }}>
                ...
                <th scope="col" style={S2_HEAD} className="px-2.5 py-2 text-left font-semibold whitespace-nowrap">Municipio</th>
                {['Departamento','Expediente N°','Monto','CC (ml)','Adoq. (m²)','OK Gob.','Exp. Doc.','Est. Jurídico','Est. Técnico','Est. Presup.','Est. General','Avance','Acciones'].map((h) => (
                ...
                return (
                  <tr key={m.id} className="border-b border-slate-100 hover:bg-sky-50/30 transition-colors">
                    ...
                    <td className="px-2.5 py-1.5 whitespace-nowrap">
                      ...
                      <button onClick={() => setEditTarget(m)} className="p-1 rounded text-gray-500 hover:text-gov-navy hover:bg-sky-50 transition-colors ml-0.5" aria-label={`Editar ${m.municipio}`} title="Editar">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      {canManage && (
                        <>
                          {confirmDeleteId === m.id ? (
                            ...
                            <button onClick={() => setConfirmDeleteId(m.id)} className="p-1 rounded text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors ml-0.5" aria-label={`Eliminar ${m.municipio}`} title="Eliminar">
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            </button>
                          )}
                        </>
