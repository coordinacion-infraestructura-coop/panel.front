...
function EditModal({
  ...
  municipio: MunicipioCC; estados: EstadoCC[]
  onSave: (data: MunicipioCCUpdate) => void; onClose: () => void; isSaving: boolean
      ...
      <div className="bg-white rounded-lg w-[740px] max-w-[97vw] max-h-[92vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label htmlFor={`${uid}-ml`} className={lbl}>Cordón-Cuneta (ml)</label>
              <input id={`${uid}-ml`} type="number" className={inp} value={form.cordon_cuneta_ml ?? ''} onChange={(e) => set('cordon_cuneta_ml', e.target.value ? Number(e.target.value) : null)} />
            </div>
            <div>
              <label htmlFor={`${uid}-m2`} className={lbl}>Adoquinado (m²)</label>
...
function HistorialEstadosTab({
  ...
  return (
    <ol className="space-y-3 px-5 py-4">
      {historial.map((h: EstadoHistorialCC, i: number) => (
...
// ── Checklist Técnico (resumen de solo lectura del módulo nuevo, spec-checklist-tecnico-dgv) ──
// Reemplaza al viejo espejo de Google Sheet (retirado — ver docs/files/auditoria-codigo.md):
// esta pestaña ahora lee EN VIVO lo que el área técnica carga en /vivienda/checklist-tecnico
// (mismo entidad_id que el municipio de Cordón Cuneta, programa='cc'). Se edita desde ese panel,
// no desde acá — el área administrativa solo mira el resumen.
...
function ChecklistTecnicoResumenTab({ municipioId }: { municipioId: string }) {
  const { data: checklist, isLoading } = useQuery({
    queryKey: ['checklist-tecnico-resumen', 'cc', municipioId],
    queryFn: () => checklistTecnicoApi.getChecklist('cc', municipioId),
  })
  const { data: catalogos } = useQuery({
    queryKey: ['checklist-catalogos'],
    queryFn: checklistTecnicoApi.getCatalogos,
  })
  const { data: obsObra } = useQuery({
    queryKey: ['checklist-tecnico-obs-obra-resumen', 'cc', municipioId],
    queryFn: () => checklistTecnicoApi.getObsObra('cc', municipioId),
    enabled: !!checklist,
  })
  ...
  if (!checklist) {
    return (
      <div className="px-5 py-8 text-center">
        <p className="text-sm text-gray-400">El área técnica todavía no cargó este municipio en el Checklist Técnico.</p>
      </div>
    )
  }

  const estados = catalogos?.estados_expediente ?? []
  const visitados = new Set(checklist.estados_visitados)
  const excepcionVisitados = estados.filter((e) => e.en_ruta === false && visitados.has(e.id))

  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
      <div>
        <p className="text-xs font-bold uppercase tracking-wide text-gov-navy mb-1.5">Estado del expediente</p>
        {checklist.estado_expediente_label ? (
          <div className="flex flex-wrap items-center gap-1.5">
            {excepcionVisitados.map((e) => (
              <span key={e.id} className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800">
                {e.label}
              </span>
            ))}
            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-gov-navy text-white">
              {checklist.estado_expediente_label}
            </span>
          </div>
        ) : (
      ...
      {(checklist.fecha_radicacion || checklist.reparticion_label) && (
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-wide text-gov-navy mb-1.5">Documentación a presentar</p>
        <ul className="space-y-1">
          {checklist.items.map((item) => (
            <li key={`${item.item_num}-${item.sub_item_num ?? ''}`} className="flex items-center justify-between gap-2 text-xs">
              <span className="text-gray-600">
                {item.sub_item_num ? <span className="pl-3">· {item.label}</span> : <>{item.item_num}. {item.label}</>}
              </span>
              <span
                className="flex-shrink-0 text-[11px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap"
                style={{ background: item.bg, color: item.text_color }}
              >
                {item.item_estado_label}
              </span>
            </li>
          ))}
        </ul>
      ...
      {checklist.hitos && (
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-gov-navy mb-1.5">Ejecución de obra</p>
          <div className="grid grid-cols-2 gap-2">
            {checklist.hitos.map((h) => (
              <div key={h.tipo} className="bg-slate-50 rounded px-2.5 py-2 text-xs">
                <div className="text-gray-400 uppercase text-[10px] font-bold">{h.label}</div>
                <div className="font-semibold text-gov-blue">{h.monto !== null ? fmtMonto(h.monto) : '—'}</div>
                <div className={`text-[10px] mt-0.5 ${h.fecha_acreditado ? 'text-green-600 font-semibold' : 'text-gray-400'}`}>
                  {h.fecha_acreditado ? `Acreditado ${new Date(h.fecha_acreditado + 'T00:00:00').toLocaleDateString('es-AR')}` : 'Pendiente'}
                </div>
              </div>
            ))}
          </div>
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-wide text-gov-navy mb-1.5">Observaciones de obra</p>
        {!obsObra || obsObra.length === 0 ? (
          <p className="text-xs text-gray-400">Sin observaciones de obra cargadas.</p>
        ) : (
          <ul className="space-y-2 max-h-48 overflow-y-auto">
            {obsObra.map((o) => (
              <li key={o.id} className="text-xs">
                <div className="font-bold text-gov-navy">{new Date(o.fecha + 'T00:00:00').toLocaleDateString('es-AR')}</div>
                <p className="text-gray-600">{o.descripcion}</p>
                {(o.created_by_nombre || o.created_by) && (
                  <p className="text-[10px] text-gray-400">{o.created_by_nombre || o.created_by}</p>
                )}
              </li>
            ))}
          </ul>
      ...
      <p className="text-[10px] text-gray-300 text-right pt-1">
        Datos de solo lectura — se cargan en Checklist Técnico. Última actualización {fmtSincronizado(checklist.updated_at)}
        {checklist.updated_by ? ` (${checklist.updated_by})` : ''}.
      </p>
    </div>
  )
}
...
function DetailPanel({
  municipio, estados, onClose,
}: {
  municipio: MunicipioCC; estados: EstadoCC[]; onClose: () => void
}) {
  const uid = useId()
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md bg-white shadow-2xl flex flex-col" role="dialog" aria-modal="true" aria-labelledby={`${uid}-t`}>
        ...
        {/* Tab: Checklist Técnico (resumen del módulo nuevo, solo lectura) */}
        {tab === 'checklist' && <ChecklistTecnicoResumenTab municipioId={municipio.id} />}
      </div>
    </>
  )
}
...
function GestionarEstadosModal({
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: EstadoCCUpdate }) => cordonCunetaApi.updateEstado(id, data),
    onSuccess: () => {
  ...
  const createMut = useMutation({
    mutationFn: (data: EstadoCCCreate) => cordonCunetaApi.createEstado(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cordon-cuneta'] })
      setShowNew(false)
      setCreateError(null)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" role="dialog" aria-modal="true" aria-labelledby={`${uid}-t`}>
      <div className="bg-white rounded-lg w-[640px] max-w-[97vw] max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="text-white px-4 py-3 flex items-center gap-3 rounded-t-lg sticky top-0 z-10" style={{ background: 'var(--color-gov-navy)' }}>
          <h3 id={`${uid}-t`} className="flex-1 font-semibold text-sm">Gestionar estados — Cordón Cuneta</h3>
...
function AgregarMunicipioModal({
  ...
  const createMut = useMutation({
    mutationFn: (data: MunicipioCCCreate) => cordonCunetaApi.createMunicipio(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cordon-cuneta'] })
      onClose()
    },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50" role="dialog" aria-modal="true" aria-labelledby={`${uid}-t`}>
      <div className="bg-white rounded-lg w-[560px] max-w-[97vw] max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="text-white px-4 py-3 flex items-center gap-3 rounded-t-lg sticky top-0 z-10" style={{ background: 'var(--color-gov-navy)' }}>
          <h3 id={`${uid}-t`} className="flex-1 font-semibold text-sm">Agregar localidad — Cordón Cuneta</h3>
          <button onClick={onClose} className="text-sky-300 hover:text-white text-xl leading-none" aria-label="Cerrar">✕</button>
        </div>
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
              ...
              {geoLoading
                ...
                : (
                  <select
                    ...
                    onChange={(e) => {
                      setForm((p) => ({ ...p, departamento: e.target.value, municipio: '' }))
                    }}
                  >
        ...
        <div className="px-5 py-3 border-t border-slate-100 flex justify-end gap-3">
          ...
          <button onClick={() => createMut.mutate(form)} disabled={createMut.isPending || !form.municipio.trim()} className="px-5 py-1.5 rounded text-sm text-white disabled:opacity-50 transition-colors hover:opacity-90" style={{ background: 'var(--color-gov-navy)' }}>
...
export function CordonCunetaPage() {
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) => cordonCunetaApi.deleteMunicipio(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cordon-cuneta'] })
  ...
  const filtered = useMemo(() => {
    ...
    return municipios.filter((m) => {
      if (q && !`${m.municipio} ${m.expediente ?? ''} ${m.doc_exp ?? ''}`.toLowerCase().includes(q)) return false
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {[
          { label: 'Municipios', value: String(municipios.length), color: 'var(--color-gov-navy)' },
          { label: 'Monto Total', value: fmtMontoResumen(montoTotal), color: 'var(--color-gov-blue)' },
          { label: 'OK Min. de Gobierno', value: String(conOkGob), color: '#22c55e' },
          { label: 'Sin Documentación', value: String(sinDoc), color: '#ef4444' },
          { label: 'Cordón-Cuneta (ml)', value: fmtMl(ccMlTotal), color: 'var(--color-gov-navy)' },
          { label: 'Adoquinado (m²)', value: fmtMl(adM2Total), color: 'var(--color-gov-navy)' },
        ].map((c) => (
      ...
      <div className="bg-white border border-slate-200 rounded-t-md px-4 py-3 flex flex-wrap gap-x-4 gap-y-2 items-center">
        <div className="flex items-center gap-2">
          ...
          <input id={searchId} type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Municipio, expediente, observaciones..." className="border border-slate-200 rounded px-2 py-1.5 text-sm w-56 focus:outline-none focus:ring-2 focus:ring-gov-cyan" aria-label="Buscar municipio" />
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor={okId} className="text-xs font-bold uppercase text-gray-500 whitespace-nowrap">OK Gobernador</label>
      ...
      <div className="bg-white rounded-b-md shadow-sm border border-t-0 border-slate-200 overflow-hidden mb-6">
        <div className="bg-slate-50 px-4 py-2 border-b border-slate-200">
          <span className="text-xs font-bold text-gov-navy">Convenios con Municipios — Provincia de Córdoba</span>
        </div>
                ...
                return (
                  <tr key={m.id} className="border-b border-slate-100 hover:bg-sky-50/30 transition-colors">
                    ...
                    <td className="px-2.5 py-1.5 whitespace-nowrap">
                      <button onClick={() => setDetailTarget(m)} className="p-1 rounded text-gray-500 hover:text-gov-cyan hover:bg-sky-50 transition-colors" aria-label={`Ver historial de ${m.municipio}`} title="Ver historial / comunicaciones">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                      </button>
                      {canEdit && (
                        <button onClick={() => { setEditTarget(m); setEditError(null) }} className="p-1 rounded text-gray-500 hover:text-gov-navy hover:bg-sky-50 transition-colors ml-0.5" aria-label={`Editar ${m.municipio}`} title="Editar">
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                      )}
                      {canManage && (
                          ...
                          {confirmDeleteId === m.id ? (
                            <span className="inline-flex flex-col items-end gap-1 ml-0.5">
                              <span className="inline-flex items-center gap-1">
                                <button onClick={() => deleteMut.mutate(m.id)} disabled={deleteMut.isPending} className="px-1.5 py-0.5 text-[10px] font-semibold text-white bg-red-500 hover:bg-red-600 rounded disabled:opacity-50 transition-colors">{deleteMut.isPending ? '...' : 'Sí'}</button>
                              ...
                              {deleteError && confirmDeleteId === m.id && (
                                <span role="alert" className="text-[10px] text-red-600 whitespace-nowrap">{deleteError}</span>
                              )}
                            </span>
                          ) : (
                            <button onClick={() => { setConfirmDeleteId(m.id); setDeleteError(null) }} className="p-1 rounded text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors ml-0.5" aria-label={`Eliminar ${m.municipio}`} title="Eliminar">
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            </button>
                          )}
                        </>
